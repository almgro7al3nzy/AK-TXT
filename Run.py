...
[bold white]Views 👁️:[bold yellow] +1000""", width=56, style="bold bright_white", title="[bold bright_white][ Sukses ]"))
...
[bold white]Link 📎:[bold red] {video_url}
[bold white]Views 👁️:[bold green] +{self.views_sent}""", width=56, style="bold bright_white", title="[bold bright_white][ Sukses ]"))
                    printf(f"[bold bright_white]   ───>[bold green] TRY SENDING VIEWS AGAIN!           ", end='\r')
                    time.sleep(2.5)
                    self.MENGIRIMKAN_TAMPILAN(video_form, post_action, video_url)
                else:
                ...
                printf(f"[bold bright_white]   ───>[bold red]  تخضع للبريد العشوائي أو الحد         ", end='\r')
                time.sleep(2.5)
                self.DELAY(0, 500)
                return (False)
            else: # YOU CAN DEBUGGING IF THIS ERROR HAPPENS!
                printf(f"[bold bright_white]   ───>[bold red] فشل في الحصول على نموذج المشاهدات    ", end='\r')
                time.sleep(3.5)
                COOKIES.update(
                    {
                        "Cookie": None
    ...
    def DELAY(self, menit, detik):
        ...
        while (self.total):
            ...
            printf(f"[bold bright_white]   ───>[bold white] 📟 [bold green] {menit:02d}:{detik:02d}[bold white] ✅ :-[bold green]{len(SUKSES)}[bold white] ❌ :-[bold red]{len(GAGAL)}          ", end='\r')
            time.sleep(1)
            self.total -= 1
        return ("0_0")
    ...
    def __init__(self):
        try:
            ...
            if 'devf5r.com' in str(video_url) or '/video/' in str(video_url):
                ...
                printf(Panel(f"[bold red]Please fill in the F5R video link correctly, make sure you take the video link in the browser!", width=56, style="bold bright_white", title="[bold bright_white][ Link Salah ]"))
                sys.exit()
...
[bold white]  \/_____/   \/_____/   \/_/     \/_____/   \/_____/
        [underline green]Free F5R Views - Coded by Abbas Al Khalifa""", width=56, style="bold bright_white")
        )
        return (True)
...
if __name__ == '__main__':
    try:
        ...
        if os.path.exists("Penyimpanan/Subscribe.json") == False:
            youtube_url = json.loads(requests.get('https://raw.githubusercontent.com/almgro7al3nzy/f5r/refs/heads/main/Youtube.json').text)['Link']
